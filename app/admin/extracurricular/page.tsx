...
export default function ExtracurricularListPage() {
  ...
  const fetchExtracurricular = async () => {
    try {
      ...
      setExtracurricular(response.data as unknown as Extracurricular[]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!token) return;
    if (!confirm("Are you sure you want to delete this extracurricular entry?"))
      return;

    try {
      await extracurricularAPI.delete(token, id);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <Link
          href="/admin/extracurricular/create"
            ...
            {extracurricular.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-6 py-8 text-center text-gray-400">
                  No extracurricular entries found. Add your first one!
                </td>
              </tr>
            ) : (
              extracurricular.map((item) => (
                <tr key={item._id} className="hover:bg-gray-800/50">
                  ...
                  <td className="px-6 py-4 text-right space-x-4">
                    <button
                      onClick={() =>
                        router.push(`/admin/extracurricular/edit/${item._id}`)
                      }
